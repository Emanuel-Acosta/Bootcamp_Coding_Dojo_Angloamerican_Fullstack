...
def loteria():
    ...
    cartas_aleatorias = random.sample(cartas, total_casillas)
    return render_template('index.html', casillas = total_casillas, filas = fila , columnas = columna, cartas=cartas_aleatorias)


@app.route('/loteria/<int:fila>')
def loterio_filas(fila):
    columna = 4
    total_casillas = fila * columna
    cartas_aleatorias = random.sample(cartas, total_casillas)
    return render_template('index.html', casillas = total_casillas, filas = fila, columnas = columna,cartas=cartas_aleatorias )
...
def loteria_filas_columnas(fila,columna):
    total_casillas = fila * columna
    cartas_aleatorias = random.sample(cartas, total_casillas)
    return render_template('index.html', casillas = total_casillas, filas = fila, columnas = columna,cartas=cartas_aleatorias)
